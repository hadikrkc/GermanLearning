...
export const NavNotification = ({mode}) => {
	...
	const getNotificationBody = list => {
			...
			renderItem={item => (
				<List.Item className="list-clickable">
					...
					<div className="mr-3">
						...
						<span className="text-gray-light">{t(item.descKey)}</span>
					</div>
		...
		<div className="empty-notification">
			...
			<p className="mt-3">{t('notification.empty')}</p>
		</div>;
	}
	...
	const notificationList = (
		<div style={{maxWidth: 300}}>
			<div className="border-bottom d-flex justify-content-between align-items-center px-3 py-2">
				<h4 className="mb-0">{t('notification.title')}</h4>
				<Button className="text-primary" type="text" onClick={() => setData([])} size="small">{t('notification.clear')}</Button>
			</div>
			...
			</div>
			{
				...
				<div className="px-3 py-2 border-top text-center">
					<a className="d-block" href="#/">{t('notification.viewAll')}</a>
				</div>
				:
				null
			}
