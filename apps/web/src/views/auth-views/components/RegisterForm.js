...
const rules = {
  displayName: [{ required: true, message: 'Please enter your full name' }],
  email: [
...
export const RegisterForm = (props) => {
  ...
  useEffect(() => {
    if (token !== null && allowRedirect) {
      navigate(redirect || '/app/onboarding');
    }
    if (showMessage) {
  ...
  return (
    <>
      ...
      <Form form={form} layout="vertical" name="register-form" onFinish={onSignUp}>
        <Form.Item name="displayName" label="Full Name" rules={rules.displayName} hasFeedback>
          <Input prefix={<UserOutlined className="text-primary" />} placeholder="Your full name" />
        </Form.Item>
        <Form.Item name="email" label="Email" rules={rules.email} hasFeedback>
          <Input prefix={<MailOutlined className="text-primary" />} />
        </Form.Item>
        <Form.Item name="role" label="I am a" rules={rules.role}>
